...
def upgrade() -> None:
    ...
    op.execute("ALTER TABLE staff ALTER COLUMN store_id TYPE VARCHAR USING store_id::text")
    op.execute("ALTER TABLE users ALTER COLUMN store_id TYPE VARCHAR USING store_id::text")
    op.execute("ALTER TABLE appointments ALTER COLUMN staff_id TYPE VARCHAR USING staff_id::text")
    op.execute("ALTER TABLE appointments ALTER COLUMN service_id TYPE VARCHAR USING service_id::text")
    op.execute("ALTER TABLE appointments ALTER COLUMN store_id TYPE VARCHAR USING store_id::text")
    op.execute("ALTER TABLE schedules ALTER COLUMN staff_id TYPE VARCHAR USING staff_id::text")
    op.execute("ALTER TABLE schedules ALTER COLUMN store_id TYPE VARCHAR USING store_id::text")
    op.execute("ALTER TABLE staff_services ALTER COLUMN staff_id TYPE VARCHAR USING staff_id::text")
    op.execute("ALTER TABLE staff_services ALTER COLUMN service_id TYPE VARCHAR USING service_id::text")
    op.execute("ALTER TABLE audit_logs ALTER COLUMN actor_id TYPE VARCHAR USING actor_id::text")
    op.execute("ALTER TABLE services ALTER COLUMN store_id TYPE VARCHAR USING store_id::text")
    op.execute("ALTER TABLE budgets ALTER COLUMN store_id TYPE VARCHAR USING store_id::text")
    op.execute("ALTER TABLE store_schedules ALTER COLUMN store_id TYPE VARCHAR USING store_id::text")
    ...
    op.add_column('appointments', sa.Column('client_name', sa.String(length=255), nullable=False, server_default='Desconocido'))
    op.add_column('appointments', sa.Column('client_email', sa.String(length=255), nullable=True))
    op.add_column('appointments', sa.Column('client_phone', sa.String(length=50), nullable=True))
    
    op.alter_column('appointments', 'starts_at', type_=sa.DateTime(), existing_type=postgresql.TIMESTAMP(timezone=True))
    op.alter_column('appointments', 'created_at', type_=sa.DateTime(), existing_type=postgresql.TIMESTAMP(timezone=True))
    op.alter_column('appointments', 'updated_at', type_=sa.DateTime(), existing_type=postgresql.TIMESTAMP(timezone=True))

    op.add_column('staff', sa.Column('first_name', sa.String(length=100), nullable=False, server_default=''))
    op.add_column('staff', sa.Column('last_name', sa.String(length=100), nullable=False, server_default=''))
    op.add_column('staff', sa.Column('email', sa.String(length=255), nullable=False, server_default=''))
    op.add_column('staff', sa.Column('service_ids', sa.JSON(), nullable=False, server_default='[]'))
    
    op.alter_column('staff', 'display_name', type_=sa.String(length=100), existing_type=sa.VARCHAR(length=255))
    op.alter_column('staff', 'created_at', type_=sa.DateTime(), existing_type=postgresql.TIMESTAMP(timezone=True))
    op.alter_column('staff', 'updated_at', type_=sa.DateTime(), existing_type=postgresql.TIMESTAMP(timezone=True))

    op.add_column('staff_services', sa.Column('rating', sa.Numeric(precision=2, scale=1), nullable=True))
    
    op.add_column('users', sa.Column('full_name', sa.String(length=255), nullable=False, server_default=''))
    op.alter_column('users', 'created_at', type_=sa.DateTime(), existing_type=postgresql.TIMESTAMP(timezone=True))
    op.alter_column('users', 'updated_at', type_=sa.DateTime(), existing_type=postgresql.TIMESTAMP(timezone=True))
    ...
    op.create_index(op.f('ix_appointment_blocks_end_time'), 'appointment_blocks', ['end_time'], unique=False)
    op.create_index(op.f('ix_appointment_blocks_id'), 'appointment_blocks', ['id'], unique=False)
    op.create_index(op.f('ix_appointment_blocks_staff_id'), 'appointment_blocks', ['staff_id'], unique=False)
    op.create_index(op.f('ix_appointment_blocks_start_time'), 'appointment_blocks', ['start_time'], unique=False)
    op.create_index(op.f('ix_appointment_blocks_store_id'), 'appointment_blocks', ['store_id'], unique=False)
