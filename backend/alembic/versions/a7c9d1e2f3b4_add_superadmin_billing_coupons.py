...
def _create_global_only_policy(table_name: str) -> None:
    ...
    op.execute(
        f"DROP POLICY IF EXISTS {table_name}_global_admin_policy ON {table_name}"
    )
    op.execute(
        f"""
...
def upgrade() -> None:
    ...
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_users_is_global_admin ON users (is_global_admin)"
    )
    ...
    op.create_table(
        ...
        *_timestamp_columns(),
        ...
        sa.PrimaryKeyConstraint("id"),
        sa.CheckConstraint(
            "coupon_type IN ('percent', 'fixed')", name="ck_saas_coupons_type"
        ),
        sa.CheckConstraint("value > 0", name="ck_saas_coupons_value_positive"),
        sa.CheckConstraint(
            "current_uses >= 0", name="ck_saas_coupons_current_uses_non_negative"
        ),
        sa.CheckConstraint(
            "max_uses IS NULL OR max_uses > 0", name="ck_saas_coupons_max_uses_positive"
        ),
        sa.CheckConstraint(
            "valid_until IS NULL OR valid_from IS NULL OR valid_from < valid_until",
            name="ck_saas_coupons_valid_window",
        ),
    )
    op.create_index("ix_saas_coupons_code", "saas_coupons", ["code"], unique=True)
    op.create_index(
        "ix_saas_coupons_created_by_id", "saas_coupons", ["created_by_id"], unique=False
    )
    ...
    op.create_table(
        ...
        *_timestamp_columns(),
        ...
        sa.PrimaryKeyConstraint("id"),
        sa.CheckConstraint(
            "base_amount >= 0", name="ck_store_subscriptions_base_non_negative"
        ),
        sa.CheckConstraint(
            "discount_amount >= 0", name="ck_store_subscriptions_discount_non_negative"
        ),
        sa.CheckConstraint(
            "total_amount >= 0", name="ck_store_subscriptions_total_non_negative"
        ),
        sa.CheckConstraint(
            "discount_amount <= base_amount",
            name="ck_store_subscriptions_discount_lte_base",
        ),
    )
    op.create_index(
        "ix_store_subscriptions_store_id",
        "store_subscriptions",
        ["store_id"],
        unique=False,
    )
    op.create_index(
        "ix_store_subscriptions_plan_id",
        "store_subscriptions",
        ["plan_id"],
        unique=False,
    )
    op.create_index(
        "ix_store_subscriptions_coupon_id",
        "store_subscriptions",
        ["coupon_id"],
        unique=False,
    )
    ...
    op.create_table(
        ...
        *_timestamp_columns(),
        ...
        sa.PrimaryKeyConstraint("id"),
        sa.CheckConstraint(
            "base_amount >= 0", name="ck_coupon_redemptions_base_non_negative"
        ),
        sa.CheckConstraint(
            "discount_amount >= 0", name="ck_coupon_redemptions_discount_non_negative"
        ),
        sa.CheckConstraint(
            "final_amount >= 0", name="ck_coupon_redemptions_final_non_negative"
        ),
    )
    op.create_index(
    ...
    op.create_index(
        "ix_coupon_redemptions_store_id",
        "coupon_redemptions",
        ["store_id"],
        unique=False,
    )
    ...
    op.create_index(
        "ix_coupon_redemptions_redeemed_by_id",
        "coupon_redemptions",
        ["redeemed_by_id"],
        unique=False,
    )
    op.create_index(
        "ix_coupon_redemptions_coupon_store",
        "coupon_redemptions",
        ["coupon_id", "store_id"],
        unique=False,
    )
...
def downgrade() -> None:
    op.execute(
        "DROP POLICY IF EXISTS coupon_redemptions_rls_policy ON coupon_redemptions"
    )
    op.execute(
        "DROP POLICY IF EXISTS store_subscriptions_rls_policy ON store_subscriptions"
    )
    ...
    op.drop_index(
        "ix_coupon_redemptions_redeemed_by_id", table_name="coupon_redemptions"
    )
    op.drop_index(
        "ix_coupon_redemptions_subscription_id", table_name="coupon_redemptions"
    )
