...
| `PATCH` | `/appointments/{id}/reschedule` | Reprograma un turno (cancela el anterior y crea uno nuevo atómicamente). `new_starts_at` entre hace 2 anios y dentro de 2 anios: la tienda puede mover un turno a un horario que ya paso (decision del dueno, 2026-09-25), sin mail `appointment.rescheduled` en ese caso; el cliente reprograma por el portal, solo al futuro. Con cobro vivo (`pending_payment` o un link de pago en `pending` o `rejected`), la reprogramacion vence el cobro del original en la misma transaccion y el turno nuevo nace sin cobro: un confirmado sigue `confirmed` y un `pending_payment` pasa a `pending` sin sena requerida y con `expires_at` = el nuevo inicio (no conserva la retencion del original; pierde la sena: decision pendiente del dueno) (antes: 409 `PAYMENT_APPOINTMENT_REQUIRES_RELEASE`; decision del dueno, 2026-09-25). |
...
| `POST` | `/payments/preferences/{appointment_id}` | Link de pago del panel (admin, superadmin, profesional; modulo `payments`). Lockea el turno antes de tocar el cobro (orden turno -> pago) y solo acepta turnos `pending`, `confirmed` o `pending_payment`: sobre uno cancelado, completado, ausente o vencido responde 409 `APPOINTMENT_NOT_PAYABLE` y no crea cobro. Si el turno se cancela mientras Mercado Pago crea el link, el link no se guarda, se manda a vencer por el outbox y tambien es 409 `APPOINTMENT_NOT_PAYABLE` (revision de perf/f4-pay, 2026-09-25). |
...
- **Cancelar desde el panel un turno `pending_payment` (o con un link de pago vivo):** ya no responde 409 `PAYMENT_APPOINTMENT_REQUIRES_RELEASE`. Responde 200 y vence el cobro. El boton "Cancelar" ya no tiene que derivar a "Liberar" ni esconderse para el profesional. Reprogramar desde el panel tampoco da ese 409: vence el cobro y el turno nuevo nace sin cobro (`pending` si el original era `pending_payment`).
...
- **Link de pago del panel:** sobre un turno terminal (cancelado, completado, ausente, vencido), o si el turno se cancela mientras se genera, `POST /payments/preferences/{id}` responde 409 `APPOINTMENT_NOT_PAYABLE`. Lo mismo `POST /payments/{appointment_id}/manual-confirm` (registrar un cobro a mano) sobre un turno cancelado, vencido, completado o ausente (revision de perf/f4-pay, 2026-09-25): para registrar un cobro de un turno ya atendido, hacerlo antes de marcarlo completado.
- **Fiado sobre el personal:** leer o cargar fiado sobre una cuenta del personal o de un admin responde 404 `RESOURCE_NOT_FOUND`.
