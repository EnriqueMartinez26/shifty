...
- **Reprogramar un turno con sena pendiente (decision del dueno 2026-09-25: opcion A):** `PATCH /appointments/{id}/reschedule` sobre un turno `pending_payment` responde 409 `DEPOSIT_PENDING_RESCHEDULE_DENIED` ("Cobra la sena o cancela el turno antes de moverlo"). El panel muestra ese mensaje tal cual y ofrece las dos salidas: cobrar la sena (link o cobro manual) y recien despues mover, o cancelar. No es un error del sistema: no reintentar.
