...
| `PATCH` | `/appointments/{id}/reschedule` | Reprograma un turno (cancela el anterior y crea uno nuevo atómicamente). `new_starts_at` entre hace 2 anios y dentro de 2 anios: la tienda puede mover un turno a un horario que ya paso (decision del dueno, 2026-09-25), sin mail `appointment.rescheduled` en ese caso; el cliente reprograma por el portal, solo al futuro. |
