...
- Las claves de Redis del panel llevan la tienda adelante desde perf/f4-back (`panel:`, `panel-reschedule:`, `panel-client:` + `store_id`). Un reintento idempotente que cruza el deploy que cambio el formato no encuentra la respuesta cacheada: recibe 409 (`RESOURCE_CONFLICT`, por el unico de `appointments.idempotency_key`) en vez del turno original. Es la ventana de un reintento en curso durante el deploy; no se pierde ni se duplica nada.
