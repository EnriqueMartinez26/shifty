...
| `GET` / `POST` | `/public/unsubscribe` | Baja del mail promocional "volve a reservar" (art. 27 Ley 25.326): el link va en ese mail, firmado (HMAC con clave derivada de `SECRET_KEY`) y valido 90 dias, por cliente y tienda. 200 `{status: "unsubscribed"}` neutro e idempotente; link adulterado o vencido: 400 `UNSUBSCRIBE_LINK_INVALID`. `POST` con cuerpo `{token}` (1..256 caracteres) tiene la misma semantica y es el que tiene que usar la pagina del front; el `GET` con `?token=` sigue mientras el link del mail apunte a la API. Rate limit `public-read`. Los mails transaccionales no cambian. |
...
- **Baja del mail "volve a reservar":** hoy el mail trae un link a `GET {PUBLIC_API_URL}/public/unsubscribe?token=...` que responde JSON (`{status: "unsubscribed"}` o 400 `UNSUBSCRIBE_LINK_INVALID`). Los escaneres de correo abren los links GET de un mail por su cuenta: con el tiempo el link tiene que apuntar a una pagina del front (por ejemplo `/baja?token=`) que muestre un boton "Darme de baja" y recien ahi llame a `POST /public/unsubscribe` con `{token}` (misma respuesta). Cuando esa pagina exista, el link del mail pasa a apuntar al front (cambio en `modules/legal/unsubscribe.py::unsubscribe_url`) y el `GET` se puede retirar.
