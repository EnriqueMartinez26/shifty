...
- [ ] Release notes for perf/f4-back mention that panel idempotency keys in Redis are now namespaced per store: an idempotent retry of a panel self-booking or reschedule that spans the deploy gets a 409 (`APPOINTMENT_CONFLICT`, or `NO_STAFF_AVAILABLE` for auto-pick) instead of the cached response (see `docs/DOCUMENTACION_TURNERO.md`, "Idempotencia y Resiliencia").
