...
export const UserManagementContainer: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-20 rounded-[3rem]"
             style={{ background: 'white', border: `1px solid ${colors2000s.border.light}`, boxShadow: colors2000s.shadows.insetDark }}>
          <Loader2 className="w-12 h-12 animate-spin mb-4" style={{ color: colors2000s.orange.accent }} />
          <p className="text-xs font-black uppercase tracking-widest" style={{ color: colors2000s.text.secondary }}>Cargando usuarios...</p>
        </div>
      ) : filteredUsers?.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 rounded-[3rem] text-center"
             style={{ background: 'white', border: `1px solid ${colors2000s.border.light}`, boxShadow: colors2000s.shadows.outer }}>
          <div className="w-20 h-20 rounded-2xl flex items-center justify-center mx-auto mb-6 shadow-inner" style={{ background: colors2000s.bg.disabled }}>
            <UserIcon size={40} className="opacity-25" style={{ color: colors2000s.text.primary }} />
          </div>
          <h3 className="text-xl font-black uppercase" style={{ color: colors2000s.text.primary }}>No se encontraron resultados</h3>
          <p className="text-xs font-bold max-w-xs mx-auto mt-2" style={{ color: colors2000s.text.secondary }}>Probá con otro término de búsqueda o agregá un nuevo usuario.</p>
        </div>
      ) : (
