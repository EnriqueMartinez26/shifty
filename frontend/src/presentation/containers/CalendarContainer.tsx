...
export const CalendarContainer: React.FC = () => {
  ...
  const unifiedEvents = useMemo<UnifiedCalendarEvent[]>(() => {
    const appointmentEvents: UnifiedCalendarEvent[] = (agendaQuery.data || []).map((appointment: CalendarAgendaAppointment) => ({
    ...
    const blockEvents: UnifiedCalendarEvent[] = blocksInRange.map((block) => {
      const staffName = staffMembers?.find((staff) => staff.id === block.staff_id)?.displayName || "Profesional";
      return {
        id: block.public_id,
        type: "block",
  ...
  const renderEventPill = (event: UnifiedCalendarEvent, compact = false) => {
    ...
    return (
      ...
      >
        <p className={`${compact ? "text-[8px]" : "text-[9px]"} font-black uppercase tracking-widest`} style={{ color: style.text }}>
          {event.type === "block" ? "Bloqueo" : event.status}
        </p>
        <p className={`${compact ? "text-[11px]" : "text-xs"} font-black`} style={{ color: colors2000s.text.primary }}>
          {event.title}
        </p>
              ...
              {staffMembers?.map((staff, idx) => (
                <div key={staff.id} className="flex-1 min-w-[150px] p-4 text-center border-r" style={{ borderColor: colors2000s.border.light }}>
                  <div className="w-10 h-10 rounded-full text-white flex items-center justify-center mx-auto mb-2 font-black text-xs shadow-md" style={{ background: idx % 2 === 0 ? "linear-gradient(180deg, #3b82f6 0%, #2563eb 100%)" : `linear-gradient(180deg, ${colors2000s.orange.light} 0%, ${colors2000s.orange.dark} 100%)`, border: idx % 2 === 0 ? "1px solid #2563eb" : `1px solid ${colors2000s.orange.accent}`, boxShadow: `${colors2000s.shadows.insetLight}, ${colors2000s.shadows.outer}` }}>
                    {staff.displayName.split(" ").map((word) => word[0]).join("").toUpperCase()}
                  </div>
              ...
              <div className="flex flex-1">
                {staffMembers?.map((staff) => (
                  <div key={staff.id} className="flex-1 min-w-[150px] relative border-r border-gray-50">
                    ...
                    {appointmentCards.filter((event) => event.staffId === staff.id).map((event) => {
                      ...
                      return (
                        ...
                        >
                          <div>
                            <p className="text-[8px] font-black uppercase tracking-widest mb-0.5" style={{ color: style.text }}>{event.subtitle}</p>
                            <h4 className="text-[11px] font-black uppercase truncate leading-tight" style={{ color: colors2000s.text.primary }}>{event.title}</h4>
                          </div>
                          <span className="self-start px-2 py-0.5 rounded-lg text-[8px] font-black tracking-widest uppercase border" style={{ background: "white", borderColor: colors2000s.border.default, boxShadow: colors2000s.shadows.insetDark, color: style.text }}>
  ...
  const renderRangeGrid = (compact = false) => (
    <div className={`grid ${compact ? "grid-cols-7" : "md:grid-cols-2 xl:grid-cols-4"} gap-4`}>
      {daysInRange.map((day) => {
        ...
        return (
          <div key={day.toISOString()} className="rounded-3xl p-4 bg-white" style={{ border: `1px solid ${colors2000s.border.light}`, boxShadow: colors2000s.shadows.insetDark }}>
            <div className="mb-3">
              <p className="text-[9px] font-black uppercase tracking-widest" style={{ color: colors2000s.orange.accent }}>{format(day, "EEE")}</p>
              <p className="text-lg font-black" style={{ color: colors2000s.text.primary }}>{format(day, "dd/MM")}</p>
            </div>
            <div className="space-y-2 max-h-64 overflow-auto">
              {dayEvents.slice(0, compact ? 4 : dayEvents.length).map((event) => renderEventPill(event, compact))}
              {compact && dayEvents.length > 4 && (
                <div className="text-[10px] font-black uppercase tracking-widest" style={{ color: colors2000s.text.secondary }}>
                  +{dayEvents.length - 4} eventos
                </div>
              )}
              {dayEvents.length === 0 && (
                <div className="text-[10px] font-bold uppercase tracking-widest" style={{ color: colors2000s.text.disabled }}>
                  Sin eventos
                </div>
              )}
            </div>
    ...
    <div className="space-y-6 animate-in fade-in duration-700">
      <div className="flex flex-col md:flex-row items-center justify-between gap-6 p-6 rounded-3xl" style={{ background: `linear-gradient(180deg, ${colors2000s.bg.button} 0%, ${colors2000s.bg.buttonBottom} 100%)`, border: `1px solid ${colors2000s.border.default}`, boxShadow: `${colors2000s.shadows.insetLight}, ${colors2000s.shadows.outerMedium}` }}>
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-2xl font-black uppercase tracking-tight leading-none mb-1" style={{ color: colors2000s.text.primary }}>Agenda</h2>
            <p className="text-[10px] font-black uppercase tracking-widest" style={{ color: colors2000s.text.secondary }}>Vistas dia, semana, mes y lista</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 p-2 rounded-2xl border" style={{ background: "white", borderColor: colors2000s.border.default, boxShadow: colors2000s.shadows.insetDark }}>
          <button onClick={() => setSelectedDate((prev) => subDays(prev, view === "month" ? 30 : view === "week" ? 7 : 1))} className="w-10 h-10 rounded-xl flex items-center justify-center transition-all active:scale-90" style={buttonStyles2000s.default}>
            <ChevronLeft size={20} className="text-gray-600" />
          </button>
          <div className="px-6 text-center min-w-[200px]">
            <p className="text-[9px] font-black uppercase tracking-widest mb-0.5" style={{ color: colors2000s.orange.accent }}>{VIEW_LABELS[view]}</p>
          ...
          <button onClick={() => setSelectedDate((prev) => addDays(prev, view === "month" ? 30 : view === "week" ? 7 : 1))} className="w-10 h-10 rounded-xl flex items-center justify-center transition-all active:scale-90" style={buttonStyles2000s.default}>
        ...
        <div className="flex flex-wrap items-center gap-2">
          {(Object.keys(VIEW_LABELS) as CalendarView[]).map((viewKey) => (
            <button key={viewKey} type="button" onClick={() => setView(viewKey)} className="px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest" style={view === viewKey ? buttonStyles2000s.selected : buttonStyles2000s.default}>
              {VIEW_LABELS[viewKey]}
            </button>
          ))}
      ...
      <div className="grid xl:grid-cols-[1.05fr_0.95fr] gap-6">
        ...
        <div className="p-6 rounded-3xl space-y-4" style={{ background: `linear-gradient(180deg, ${colors2000s.bg.button} 0%, ${colors2000s.bg.buttonBottom} 100%)`, border: `1px solid ${colors2000s.border.default}`, boxShadow: `${colors2000s.shadows.insetLight}, ${colors2000s.shadows.outerMedium}` }}>
          <h3 className="text-lg font-black uppercase tracking-tight" style={{ color: colors2000s.text.primary }}>Bloqueos y ausencias</h3>
          <div className="space-y-3">
            {timelineEvents.map((event) => (
              <div key={`${event.type}-${event.id}`} className="rounded-2xl p-4 bg-white flex flex-col gap-3" style={{ border: `1px solid ${colors2000s.border.light}`, boxShadow: colors2000s.shadows.insetDark }}>
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <p className="text-sm font-black" style={{ color: colors2000s.text.primary }}>{event.title}</p>
                    <p className="text-[11px] font-bold" style={{ color: colors2000s.text.secondary }}>
                      {format(event.startsAt, "dd/MM HH:mm")} - {format(event.endsAt, "HH:mm")} · {event.staffName}
                    </p>
                  </div>
                  <span className="px-2 py-1 rounded-xl text-[10px] font-black uppercase tracking-widest" style={{ background: event.type === "block" ? "#ffedd5" : "#fee2e2", color: event.type === "block" ? "#c2410c" : "#b91c1c" }}>
                    {event.type === "block" ? "Bloqueo" : "Ausencia"}
                  </span>
                </div>
                {event.type === "block" && (
                  <div className="flex flex-wrap gap-2">
                    <button type="button" onClick={() => handleEditBlock({ public_id: event.id, staff_id: event.staffId, starts_at: event.startsAt.toISOString(), ends_at: event.endsAt.toISOString(), reason: event.title })} className="px-3 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest" style={buttonStyles2000s.default}>
                      Editar
                    </button>
