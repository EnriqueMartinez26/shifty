...
const PublicBooking: React.FC = () => {
  ...
  if (isLoading) {
    return <div className="min-h-screen grid place-items-center bg-[#EEF2F6] text-sm font-black uppercase tracking-widest text-gray-500">Cargando...</div>;
  }

  if (isError || !store) {
    return <div className="min-h-screen grid place-items-center bg-[#EEF2F6] text-sm font-black uppercase tracking-widest text-red-500 font-black">Negocio no encontrado</div>;
  }
  ...
  return (
    <div className="min-h-screen bg-[#EEF2F6] py-12 px-4 sm:px-6 lg:px-8">
      ...
      {store.description && (
        <div className="max-w-2xl mx-auto mb-6 rounded-3xl p-6 text-center border"
             style={{
               background: `linear-gradient(180deg, ${colors2000s.bg.button} 0%, ${colors2000s.bg.buttonBottom} 100%)`,
               borderColor: colors2000s.border.default,
               boxShadow: `${colors2000s.shadows.insetLight}, ${colors2000s.shadows.outer}`
             }}>
          <p className="text-sm font-bold leading-relaxed" style={{ color: colors2000s.text.primary }}>
            {store.description}
          </p>
        </div>
      )}
