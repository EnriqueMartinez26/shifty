...
const PaymentsPage: React.FC = () => {
  ...
  const summaryCards = useMemo(() => {
    ...
    return [
      ...
      { label: "Total cobrado", value: currencyFmt.format(Number(summary?.total_approved_amount ?? 0)) },
    ];
  }, [summaryQuery.data]);
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1.1fr_0.9fr] gap-6">
        <div className="p-6 rounded-3xl space-y-5" style={cardStyle}>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase tracking-widest" style={{ color: colors2000s.text.secondary }}>Clave publica</label>
              <input
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase tracking-widest" style={{ color: colors2000s.text.secondary }}>Token privado</label>
              <input
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase tracking-widest" style={{ color: colors2000s.text.secondary }}>Clave de notificaciones</label>
              <input
        ...
        <div className="p-6 rounded-3xl space-y-5" style={cardStyle}>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="rounded-2xl p-4 bg-white" style={{ border: `1px solid ${colors2000s.border.light}`, boxShadow: colors2000s.shadows.insetDark }}>
              <p className="text-[10px] font-black uppercase tracking-widest text-gray-400">Por revisar</p>
            ...
            <div className="rounded-2xl p-4 bg-white" style={{ border: `1px solid ${colors2000s.border.light}`, boxShadow: colors2000s.shadows.insetDark }}>
              <p className="text-[10px] font-black uppercase tracking-widest text-gray-400">Actualizados</p>
              <p className="mt-2 text-xl font-black text-green-600">{outboxStatsQuery.data?.processed ?? 0}</p>
            </div>
          </div>

          <div className="space-y-3">
            <label className="text-[10px] font-black uppercase tracking-widest" style={{ color: colors2000s.text.secondary }}>Devolucion manual</label>
            <input value={refundForm.paymentId} onChange={(e) => setRefundForm((prev) => ({ ...prev, paymentId: e.target.value }))} className="w-full rounded-2xl px-4 py-3 font-bold outline-none" style={inputStyle} placeholder="ID del cobro" />
            ...
            <textarea value={refundForm.reason} onChange={(e) => setRefundForm((prev) => ({ ...prev, reason: e.target.value }))} className="w-full min-h-24 rounded-2xl px-4 py-3 font-bold outline-none resize-y" style={inputStyle} placeholder="Motivo de la devolucion" />
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] font-bold" style={{ color: colors2000s.text.secondary }}>
              Las promociones y los turnos por cobrar ahora viven en sus propias secciones del menu lateral.
            </p>
          </div>
        </div>
      </div>
