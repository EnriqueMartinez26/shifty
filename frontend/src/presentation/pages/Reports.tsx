...
import { Download, FileSpreadsheet, FileText, Loader2, Table2, TrendingUp, Users, Wallet } from "lucide-react";
  ...
  if (summaryQuery.isError) {
    return (
      <div className="space-y-8 animate-in fade-in duration-500">
        <div className="flex flex-wrap gap-4 items-end justify-between p-6 rounded-3xl" style={cardStyle}>
          <div>
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      >
        <div>
          <h2 className="text-2xl font-black uppercase tracking-tight" style={{ color: colors2000s.text.primary }}>Reportes</h2>
          <p className="text-xs font-bold" style={{ color: colors2000s.text.secondary }}>Analiza turnos, clientes, servicios y deuda.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
        ...
        <div className="p-5 rounded-2xl" style={cardStyle}>
          ...
          <p className="text-2xl font-black mt-1" style={{ color: colors2000s.orange.accent }}>{currencyFmt.format(stats?.total_revenue ?? 0)}</p>
        </div>
        <div className="p-5 rounded-2xl" style={cardStyle}>
          <p className="text-[10px] font-black uppercase tracking-widest" style={{ color: colors2000s.text.secondary }}>Ticket promedio</p>
          <p className="text-2xl font-black mt-1" style={{ color: colors2000s.text.primary }}>{currencyFmt.format(stats?.average_ticket ?? 0)}</p>
        </div>
        <div className="p-5 rounded-2xl" style={cardStyle}>
          <p className="text-[10px] font-black uppercase tracking-widest" style={{ color: colors2000s.text.secondary }}>Saldo en deuda</p>
          <p className="text-2xl font-black mt-1" style={{ color: colors2000s.text.primary }}>{currencyFmt.format(debtSummary?.outstanding_balance ?? 0)}</p>
        </div>
        <div className="p-5 rounded-2xl" style={cardStyle}>
          <p className="text-[10px] font-black uppercase tracking-widest flex items-center gap-2" style={{ color: colors2000s.text.secondary }}>
            <Users className="w-3.5 h-3.5" /> Clientes en rango
          </p>
          <p className="text-2xl font-black mt-1" style={{ color: colors2000s.text.primary }}>{clientStats?.total_clients ?? 0}</p>
        </div>
        <div className="p-5 rounded-2xl" style={cardStyle}>
          <p className="text-[10px] font-black uppercase tracking-widest flex items-center gap-2" style={{ color: colors2000s.text.secondary }}>
            <TrendingUp className="w-3.5 h-3.5" /> Nuevos clientes
          </p>
          <p className="text-2xl font-black mt-1" style={{ color: colors2000s.text.primary }}>{clientStats?.new_clients ?? 0}</p>
        </div>
        <div className="p-5 rounded-2xl" style={cardStyle}>
          <p className="text-[10px] font-black uppercase tracking-widest" style={{ color: colors2000s.text.secondary }}>Recurrentes</p>
          <p className="text-2xl font-black mt-1" style={{ color: colors2000s.text.primary }}>{clientStats?.returning_clients ?? 0}</p>
        </div>
        <div className="p-5 rounded-2xl" style={cardStyle}>
          <p className="text-[10px] font-black uppercase tracking-widest flex items-center gap-2" style={{ color: colors2000s.text.secondary }}>
            <Wallet className="w-3.5 h-3.5" /> Clientes con deuda
          </p>
          <p className="text-2xl font-black mt-1" style={{ color: colors2000s.text.primary }}>{debtSummary?.debtors_count ?? 0}</p>
        </div>
      </div>
      ...
      <div className="grid xl:grid-cols-3 gap-6">
        <div className="rounded-3xl overflow-hidden" style={{ background: "white", border: `1px solid ${colors2000s.border.default}`, boxShadow: colors2000s.shadows.insetDark }}>
          <div className="px-6 py-4 font-black uppercase tracking-tight text-sm" style={{ background: colors2000s.bg.disabled, borderBottom: `1px solid ${colors2000s.border.default}`, color: colors2000s.text.primary }}>
            Servicios mas vendidos
          </div>
          <div className="p-4 space-y-3">
            {(summary?.top_services || []).map((item) => (
              <div key={item.service_id} className="rounded-2xl p-4" style={{ background: colors2000s.bg.disabledBottom, border: `1px solid ${colors2000s.border.light}` }}>
                <p className="text-sm font-black" style={{ color: colors2000s.text.primary }}>{item.service_name}</p>
                ...
                <p className="text-xs font-black mt-2" style={{ color: colors2000s.orange.accent }}>{currencyFmt.format(item.revenue)}</p>
              </div>
            ))}
            {(summary?.top_services.length ?? 0) === 0 && (
              <div className="rounded-2xl p-4 text-sm font-bold" style={{ background: colors2000s.bg.disabledBottom, color: colors2000s.text.secondary }}>
        ...
        <div className="rounded-3xl overflow-hidden" style={{ background: "white", border: `1px solid ${colors2000s.border.default}`, boxShadow: colors2000s.shadows.insetDark }}>
          ...
          <div className="p-4 space-y-3">
            {(summary?.top_clients || []).map((item) => (
              <div key={item.client_id} className="rounded-2xl p-4" style={{ background: colors2000s.bg.disabledBottom, border: `1px solid ${colors2000s.border.light}` }}>
                <p className="text-sm font-black" style={{ color: colors2000s.text.primary }}>{item.client_name}</p>
                <p className="text-[11px] font-bold mt-1" style={{ color: colors2000s.text.secondary }}>
                  {item.appointments} reservas · {item.completed_appointments} completados
                </p>
                <p className="text-xs font-black mt-2" style={{ color: colors2000s.orange.accent }}>{currencyFmt.format(item.revenue)}</p>
        ...
        <div className="rounded-3xl overflow-hidden" style={{ background: "white", border: `1px solid ${colors2000s.border.default}`, boxShadow: colors2000s.shadows.insetDark }}>
          ...
          <div className="p-4 space-y-3">
            {(debtSummary?.top_debtors || []).map((item) => (
              <div key={item.client_id} className="rounded-2xl p-4" style={{ background: colors2000s.bg.disabledBottom, border: `1px solid ${colors2000s.border.light}` }}>
                <p className="text-sm font-black" style={{ color: colors2000s.text.primary }}>{item.client_name}</p>
                <p className="text-[11px] font-bold mt-1" style={{ color: colors2000s.text.secondary }}>Cliente con saldo pendiente</p>
                <p className="text-xs font-black mt-2" style={{ color: colors2000s.orange.accent }}>{currencyFmt.format(item.balance)}</p>
              </div>
            ))}
              ...
              {summary?.appointments.map((item) => (
                <tr key={item.public_id} className="hover:bg-zinc-50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <span className="px-2 py-1 rounded font-black text-[10px] uppercase" style={{ background: colors2000s.bg.disabled, color: colors2000s.text.secondary }}>
                      {item.status}
                    </span>
                  </td>
                  ...
                  <td className="px-6 py-4 font-black text-right" style={{ color: colors2000s.text.primary }}>{currencyFmt.format(item.service_price)}</td>
                </tr>
              ))}
