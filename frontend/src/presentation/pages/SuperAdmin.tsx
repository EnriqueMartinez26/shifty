...
const ActionButton: React.FC<{
  ...
  const style = tone === "primary" ? primaryActionStyle : tone === "danger" ? dangerActionStyle : secondaryActionStyle;
  return (
    <button type="button" onClick={onClick} disabled={disabled} className="disabled:opacity-50" style={style}>
      {label}
    </button>
  );
};
...
const SuperAdminPage: React.FC = () => {
  ...
  const [subscriptionForm, setSubscriptionForm] = useState<SubscriptionFormState>(createEmptySubscriptionForm());
  ...
  const openEditStoreFor = (store: SuperAdminStoreRow) => {
    setStoreForm({
      name: store.name,
      slug: store.slug,
      logo_url: overview?.store.public_id === store.public_id ? overview.store.logo_url || "" : store.logo_url || "",
      primary_color: store.primary_color,
      cancellation_hours: String(
        overview?.store.public_id === store.public_id ? overview.store.cancellation_hours : store.cancellation_hours
      ),
      buffer_minutes: String(
        overview?.store.public_id === store.public_id ? overview.store.buffer_minutes : store.buffer_minutes
      ),
  ...
  const openAssignPlanModal = () => {
    ...
    setSubscriptionForm({
      plan_id:
        ...
        activePlans[0]?.public_id ||
        "",
      status: overview?.subscription?.status || "active",
      base_amount: overview?.subscription?.base_amount ? String(overview.subscription.base_amount) : "",
  ...
  const toggleStoreActive = async (store: SuperAdminStoreRow) => {
    ...
    try {
      ...
      setFeedback({ tone: "error", text: getErrorMessage(error, "No se pudo cambiar el estado de la tienda") });
    }
  };
  ...
  const toggleUserActive = async (targetUser: SuperAdminUser) => {
    ...
    const confirmed = window.confirm(
      nextActiveState
        ? `Activar ${targetUser.email}?`
        : `Desactivar ${targetUser.email}?`
    );
    if (!confirmed) return;
    ...
    try {
      ...
      setFeedback({ tone: "error", text: getErrorMessage(error, "No se pudo cambiar el estado del usuario") });
    }
  };
  ...
  const toggleGlobalAdmin = async (targetUser: SuperAdminUser) => {
    ...
    try {
      ...
      setFeedback({ tone: "error", text: getErrorMessage(error, "No se pudo actualizar el permiso global") });
    }
  };
  ...
  return (
    <div className="space-y-6">
      {feedback ? (
        <div className="flex items-start gap-3 rounded-[1.5rem] px-5 py-4 text-sm font-bold" style={feedbackStyle}>
          <AlertTriangle className="mt-0.5 h-5 w-5 flex-shrink-0" />
          <div className="flex-1">
            <p>{feedback.text}</p>
          </div>
          <button type="button" onClick={() => setFeedback(null)} className="text-xs font-black uppercase tracking-widest">
            Cerrar
          </button>
        </div>
      ) : null}
      ...
      <section className="rounded-[2rem] p-6" style={panelStyle}>
        <div className="flex flex-col gap-6 xl:flex-row xl:items-start xl:justify-between">
          ...
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
            ...
            <ActionButton label="Editar tienda" onClick={openEditStoreModal} disabled={!selectedStore} />
            <ActionButton label="Crear admin" onClick={openCreateAdminModal} disabled={!selectedStore} />
            <ActionButton
              label="Asignar plan"
            ...
            <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
              <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-6">
                ...
                <div>
                  ...
                  <p className="font-black" style={{ color: colors2000s.text.primary }}>
                    {overview?.store.cancellation_hours ?? selectedStore.cancellation_hours}h cancel. / {overview?.store.buffer_minutes ?? selectedStore.buffer_minutes}m buffer
                  </p>
                </div>
                <div>
                  <p className="text-[10px] font-black uppercase tracking-widest" style={{ color: colors2000s.text.secondary }}>
              ...
              <div className="rounded-2xl px-4 py-3" style={{ ...scopeBadgeStyle("danger"), boxShadow: colors2000s.shadows.outer }}>
                ...
                <p className="mt-1 text-[11px] font-bold">
                  Revocar Super Admin, desactivar tienda, reemplazar suscripcion y canjear cupon tienen confirmacion y reglas de backend.
                </p>
              </div>
            </div>
          ) : (
                ...
                <div className="overflow-x-auto">
                  <table className="min-w-full text-left">
                    <thead style={{ background: colors2000s.bg.disabled }}>
                      <tr>
                        {["Tienda", "Estado", "Recordatorios", "Usuarios", "Suscripcion", "Renueva", "Acciones"].map((label) => (
                      ...
                      {storesQuery.data.map((store) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="px-4 py-4 text-[10px] font-black uppercase tracking-widest" style={{ color: colors2000s.text.secondary }}>
                              <div>Confirmacion: <span style={{ color: colors2000s.text.primary }}>{store.send_email_confirmation ? "On" : "Off"}</span></div>
                              <div className="mt-1">Reminders: <span style={{ color: colors2000s.text.primary }}>{store.send_email_reminders ? "On" : "Off"}</span></div>
                            </td>
                            <td className="px-4 py-4">
                              <p className="font-black" style={{ color: colors2000s.text.primary }}>
                ...
                {plansQuery.data?.map((plan) => (
                  <div key={plan.public_id} className="rounded-[1.5rem] p-4" style={innerCardStyle}>
                    ...
                    <div className="mt-4 grid grid-cols-3 gap-3 text-[10px] font-black uppercase tracking-widest" style={{ color: colors2000s.text.secondary }}>
                      ...
                      <div>Ciclo: <span style={{ color: colors2000s.text.primary }}>{plan.billing_interval}</span></div>
                    </div>
                      ...
                      <div key={coupon.public_id} className="rounded-[1.5rem] p-4" style={innerCardStyle}>
                        <div className="flex items-start justify-between gap-3">
                          <div>
                            ...
                            <p className="text-[10px] font-bold uppercase tracking-widest" style={{ color: colors2000s.text.secondary }}>
                              {coupon.coupon_type} · {coupon.coupon_type === "percent" ? `${coupon.value}%` : money(coupon.value, coupon.currency || "ARS")}
                            </p>
                          </div>
                        ...
                        <div className="mt-4 grid grid-cols-2 gap-3 text-[10px] font-black uppercase tracking-widest" style={{ color: colors2000s.text.secondary }}>
                          <div>Vigencia: <span style={{ color: colors2000s.text.primary }}>{formatDate(coupon.valid_from)} a {formatDate(coupon.valid_until)}</span></div>
                          <div>Canje unico: <span style={{ color: colors2000s.text.primary }}>{coupon.one_time_per_store ? "Si" : "No"}</span></div>
                        </div>
                        ...
                        {(expired || exhausted) ? (
                          <div className="mt-4 rounded-2xl px-3 py-2 text-[10px] font-black uppercase tracking-widest" style={scopeBadgeStyle("danger")}>
        ...
        <aside className="space-y-6 xl:sticky xl:top-8 xl:self-start">
          <section className="rounded-[2rem] p-6" style={panelStyle}>
            <div className="mb-4 flex items-center justify-between">
              ...
              {overviewQuery.isFetching ? <Loader2 className="h-4 w-4 animate-spin" style={{ color: colors2000s.orange.accent }} /> : null}
            </div>
            ...
            {!overview && overviewQuery.isLoading ? (
              ...
              <div className="space-y-4">
                {!selectedStore?.is_active ? (
                  <div className="rounded-[1.5rem] px-4 py-3 text-xs font-bold" style={scopeBadgeStyle("danger")}>
                    Contexto de tienda inactiva. Crear admins, asignar planes y canjear cupones puede quedar bloqueado por reglas de backend.
                  </div>
                ) : null}
                  ...
                  {overview.users.admins.length ? (
                    <div className="space-y-3">
                      {overview.users.admins.map((admin) => (
                        <div key={admin.public_id} className="rounded-2xl p-3" style={{ background: colors2000s.bg.button }}>
                          <div className="flex items-start justify-between gap-2">
                            ...
                            <span className="rounded-full px-2 py-1 text-[10px] font-black uppercase tracking-widest" style={admin.is_global_admin ? scopeBadgeStyle("danger") : scopeBadgeStyle("tenant")}>
                  ...
                  <div className="space-y-2">
                    {overview.users.users.length ? (
                      overview.users.users.map((tenantUser) => (
                        <div key={tenantUser.public_id} className="rounded-2xl px-3 py-3" style={{ background: colors2000s.bg.button }}>
                          <div className="flex items-center justify-between gap-3">
                            <div>
                              <p className="text-sm font-black" style={{ color: colors2000s.text.primary }}>
                                {[tenantUser.first_name, tenantUser.last_name].filter(Boolean).join(" ") || tenantUser.email}
                              </p>
                              <p className="text-[10px] font-bold uppercase tracking-widest" style={{ color: colors2000s.text.secondary }}>
                            ...
                            {tenantUser.is_global_admin ? (
                              ...
                              <span className="text-[10px] font-black uppercase tracking-widest" style={{ color: tenantUser.is_active ? "#15803d" : "#b91c1c" }}>
                          ...
                          <div className="mt-3 flex flex-wrap gap-2">
                            ...
                            />
                            <MiniButton
                              label={tenantUser.is_global_admin ? "Revocar SuperAdmin" : "Promover SuperAdmin"}
                              onClick={(event) => {
                                event.stopPropagation();
          ...
          <section className="rounded-[2rem] p-6" style={panelStyle}>
            <div className="flex items-start gap-3 rounded-[1.5rem] p-4" style={{ ...scopeBadgeStyle("danger"), boxShadow: colors2000s.shadows.outer }}>
              <AlertTriangle className="mt-0.5 h-5 w-5 flex-shrink-0" />
              <div>
                <p className="text-[10px] font-black uppercase tracking-widest">Estados sensibles</p>
      ...
      >
        <div className={formGridClass}>
          <div>
            ...
            <TextInput value={storeForm.name} onChange={(event) => setStoreForm((current) => ({ ...current, name: event.target.value }))} required />
          </div>
          <div>
            <FieldLabel>Slug</FieldLabel>
            <TextInput value={storeForm.slug} onChange={(event) => setStoreForm((current) => ({ ...current, slug: event.target.value }))} required />
          </div>
        </div>
        ...
        <div className={formGridClass}>
          <div>
            ...
            <div className="flex gap-3">
              <TextInput type="color" value={storeForm.primary_color} onChange={(event) => setStoreForm((current) => ({ ...current, primary_color: event.target.value }))} className="h-12 w-20 p-2" />
              <TextInput value={storeForm.primary_color} onChange={(event) => setStoreForm((current) => ({ ...current, primary_color: event.target.value }))} />
            </div>
          </div>
          <div>
            <FieldLabel>Logo URL</FieldLabel>
            <TextInput value={storeForm.logo_url} onChange={(event) => setStoreForm((current) => ({ ...current, logo_url: event.target.value }))} placeholder="https://..." />
          </div>
        </div>
        ...
        <div className={formGridClass}>
          <div>
            ...
            <TextInput type="number" min="0" value={storeForm.cancellation_hours} onChange={(event) => setStoreForm((current) => ({ ...current, cancellation_hours: event.target.value }))} required />
          </div>
          <div>
            <FieldLabel>Buffer (minutos)</FieldLabel>
            <TextInput type="number" min="0" value={storeForm.buffer_minutes} onChange={(event) => setStoreForm((current) => ({ ...current, buffer_minutes: event.target.value }))} required />
          </div>
        </div>
        ...
        <div className="grid gap-3">
          <ToggleRow
            ...
            onToggle={() => setStoreForm((current) => ({ ...current, send_email_confirmation: !current.send_email_confirmation }))}
          />
          <ToggleRow
            ...
            onToggle={() => setStoreForm((current) => ({ ...current, send_email_reminders: !current.send_email_reminders }))}
          />
          {modal === "edit-store" ? (
            <ToggleRow
              label="Tienda activa"
              ...
              onToggle={() => setStoreForm((current) => ({ ...current, is_active: !current.is_active }))}
            />
          ) : null}
        </div>
      </SuperAdminFormModal>
      ...
      >
        <div className={formGridClass}>
          <div>
            <FieldLabel>Nombre</FieldLabel>
            <TextInput value={adminForm.first_name} onChange={(event) => setAdminForm((current) => ({ ...current, first_name: event.target.value }))} required />
          </div>
          <div>
            <FieldLabel>Apellido</FieldLabel>
            <TextInput value={adminForm.last_name} onChange={(event) => setAdminForm((current) => ({ ...current, last_name: event.target.value }))} required />
          </div>
        </div>
        ...
        <div className={formGridClass}>
          <div>
            ...
            <TextInput type="email" value={adminForm.email} onChange={(event) => setAdminForm((current) => ({ ...current, email: event.target.value }))} required />
          </div>
          <div>
            <FieldLabel>Telefono</FieldLabel>
            <TextInput value={adminForm.phone} onChange={(event) => setAdminForm((current) => ({ ...current, phone: event.target.value }))} />
          </div>
        </div>

        <div>
          <FieldLabel>Password</FieldLabel>
          <TextInput type="password" value={adminForm.password} onChange={(event) => setAdminForm((current) => ({ ...current, password: event.target.value }))} minLength={8} required />
        </div>
      </SuperAdminFormModal>
      ...
      <SuperAdminFormModal
        ...
        error={modalError}
      >
        <div className="rounded-2xl px-4 py-3 text-xs font-bold" style={{ ...innerCardStyle, color: colors2000s.text.secondary }}>
          Email de acceso: <span style={{ color: colors2000s.text.primary }}>{userForm.email || "Sin email"}</span>
        </div>

        <div className={formGridClass}>
          <div>
            <FieldLabel>Nombre</FieldLabel>
            <TextInput value={userForm.first_name} onChange={(event) => setUserForm((current) => ({ ...current, first_name: event.target.value }))} />
          </div>
          <div>
            <FieldLabel>Apellido</FieldLabel>
            <TextInput value={userForm.last_name} onChange={(event) => setUserForm((current) => ({ ...current, last_name: event.target.value }))} />
          </div>
        </div>
        ...
        <div className={formGridClass}>
          <div>
            ...
            <SelectInput value={userForm.role} onChange={(event) => setUserForm((current) => ({ ...current, role: event.target.value as UserFormState["role"] }))}>
          ...
          <div>
            <FieldLabel>Telefono</FieldLabel>
            <TextInput value={userForm.phone} onChange={(event) => setUserForm((current) => ({ ...current, phone: event.target.value }))} />
          </div>
        </div>

        <div>
          <FieldLabel>Nueva password (opcional)</FieldLabel>
          <TextInput type="password" value={userForm.password} onChange={(event) => setUserForm((current) => ({ ...current, password: event.target.value }))} minLength={8} />
        </div>
      ...
      >
        <div className={formGridClass}>
          <div>
            <FieldLabel>Nombre</FieldLabel>
            <TextInput value={planForm.name} onChange={(event) => setPlanForm((current) => ({ ...current, name: event.target.value }))} required />
          </div>
          <div>
            <FieldLabel>Intervalo</FieldLabel>
            <SelectInput value={planForm.billing_interval} onChange={(event) => setPlanForm((current) => ({ ...current, billing_interval: event.target.value }))}>
        ...
        <div className={formGridClass}>
          <div>
            ...
            <TextInput type="number" min="0" step="0.01" value={planForm.price} onChange={(event) => setPlanForm((current) => ({ ...current, price: event.target.value }))} required />
          </div>
          <div>
            <FieldLabel>Moneda</FieldLabel>
            <TextInput value={planForm.currency} onChange={(event) => setPlanForm((current) => ({ ...current, currency: event.target.value.toUpperCase() }))} required />
          </div>
        </div>
        ...
        <div className={formGridClass}>
          <div>
            ...
            <TextInput type="number" min="0" value={planForm.max_staff} onChange={(event) => setPlanForm((current) => ({ ...current, max_staff: event.target.value }))} placeholder="Libre" />
          </div>
          <div>
            <FieldLabel>Max services</FieldLabel>
            <TextInput type="number" min="0" value={planForm.max_services} onChange={(event) => setPlanForm((current) => ({ ...current, max_services: event.target.value }))} placeholder="Libre" />
          </div>
        </div>
      ...
      >
        ...
        <div className={formGridClass}>
          <div>
            ...
            <SelectInput value={subscriptionForm.plan_id} onChange={(event) => setSubscriptionForm((current) => ({ ...current, plan_id: event.target.value }))} required>
          ...
          <div>
            ...
            <SelectInput value={subscriptionForm.status} onChange={(event) => setSubscriptionForm((current) => ({ ...current, status: event.target.value }))}>
        ...
        <div className={formGridClass}>
          <div>
            ...
            <TextInput type="number" min="0" step="0.01" value={subscriptionForm.base_amount} onChange={(event) => setSubscriptionForm((current) => ({ ...current, base_amount: event.target.value }))} placeholder="Usa precio del plan si queda vacio" />
          </div>
          <div>
            <FieldLabel>Moneda</FieldLabel>
            <TextInput value={subscriptionForm.currency} onChange={(event) => setSubscriptionForm((current) => ({ ...current, currency: event.target.value.toUpperCase() }))} />
          </div>
        </div>
        ...
        <div className={formGridClass}>
          <div>
            ...
            <TextInput type="datetime-local" value={subscriptionForm.current_period_start} onChange={(event) => setSubscriptionForm((current) => ({ ...current, current_period_start: event.target.value }))} />
          </div>
          <div>
            <FieldLabel>Periodo hasta</FieldLabel>
            <TextInput type="datetime-local" value={subscriptionForm.current_period_end} onChange={(event) => setSubscriptionForm((current) => ({ ...current, current_period_end: event.target.value }))} />
          </div>
        </div>
      </SuperAdminFormModal>
      ...
      >
        <div className={formGridClass}>
          <div>
            ...
            <TextInput value={couponForm.code} onChange={(event) => setCouponForm((current) => ({ ...current, code: event.target.value.toUpperCase() }))} required />
          </div>
          <div>
            <FieldLabel>Tipo</FieldLabel>
            <SelectInput value={couponForm.coupon_type} onChange={(event) => setCouponForm((current) => ({ ...current, coupon_type: event.target.value as CouponFormState["coupon_type"] }))}>
        ...
        <div className={formGridClass}>
          <div>
            ...
            <TextInput type="number" min="0" step="0.01" value={couponForm.value} onChange={(event) => setCouponForm((current) => ({ ...current, value: event.target.value }))} required />
          </div>
          <div>
            <FieldLabel>Moneda</FieldLabel>
            <TextInput value={couponForm.currency} onChange={(event) => setCouponForm((current) => ({ ...current, currency: event.target.value.toUpperCase() }))} disabled={couponForm.coupon_type === "percent"} />
          </div>
        </div>
        ...
        <div className={formGridClass}>
          <div>
            ...
            <TextInput type="number" min="1" value={couponForm.max_uses} onChange={(event) => setCouponForm((current) => ({ ...current, max_uses: event.target.value }))} placeholder="Sin limite" />
          </div>
          <div>
            <FieldLabel>Canje unico por tienda</FieldLabel>
            <SelectInput value={couponForm.one_time_per_store ? "yes" : "no"} onChange={(event) => setCouponForm((current) => ({ ...current, one_time_per_store: event.target.value === "yes" }))}>
        ...
        <div className={formGridClass}>
          <div>
            ...
            <TextInput type="datetime-local" value={couponForm.valid_from} onChange={(event) => setCouponForm((current) => ({ ...current, valid_from: event.target.value }))} />
          </div>
          <div>
            <FieldLabel>Vigente hasta</FieldLabel>
            <TextInput type="datetime-local" value={couponForm.valid_until} onChange={(event) => setCouponForm((current) => ({ ...current, valid_until: event.target.value }))} />
          </div>
        </div>
        ...
        {modal === "edit-coupon" ? (
          <ToggleRow
            ...
            onToggle={() => setCouponForm((current) => ({ ...current, is_active: !current.is_active }))}
          />
        ) : null}
      </SuperAdminFormModal>
      ...
      >
        ...
        <div>
          ...
          <SelectInput value={redeemForm.coupon_code} onChange={(event) => setRedeemForm({ coupon_code: event.target.value })} required>
            <option value="">Selecciona un cupon</option>
            {activeCoupons.map((coupon) => (
              <option key={coupon.public_id} value={coupon.code}>
                {coupon.code} · {coupon.coupon_type === "percent" ? `${coupon.value}%` : money(coupon.value, coupon.currency || "ARS")}
              </option>
            ))}
          </SelectInput>
        </div>
        ...
        <div className="rounded-2xl px-4 py-3 text-xs font-bold" style={{ ...innerCardStyle, color: colors2000s.text.secondary }}>
          El backend valida tienda activa, suscripcion vigente, expiracion del cupon y maximo de usos antes de confirmar el canje.
        </div>
      </SuperAdminFormModal>
    </div>
  );
