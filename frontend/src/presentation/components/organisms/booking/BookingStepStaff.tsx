...
export const BookingStepStaff: React.FC<BookingStepStaffProps> = ({ storePublicId, serviceId, selectedId, onSelect, onBack }) => {
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center h-48">
        ...
        <p className="text-sm font-black text-gray-500 uppercase tracking-widest">Cargando profesionales...</p>
      </div>
    );
  }
  ...
  const cards = [
    ...
    ...(staffList || []).map((staff, idx) => ({
      ...
      accent: idx % 2 === 0 ? "linear-gradient(180deg, #3b82f6 0%, #2563eb 100%)" : `linear-gradient(180deg, ${colors2000s.orange.light} 0%, ${colors2000s.orange.dark} 100%)`,
    ...
    <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-500">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          ...
          <p className="text-sm font-bold text-gray-500">Podes elegir un profesional puntual o dejar que el sistema lo asigne.</p>
        </div>
      </div>
      ...
      <div className="grid gap-4">
        {cards.map((card, idx) => {
          ...
          return (
            ...
            >
              <div className="absolute left-0 top-0 bottom-0 w-2.5" style={{ background: card.accent }} />
              <div
                ...
                style={{
                  ...
                  borderColor: isSelected ? "rgba(255,255,255,0.4)" : idx % 2 === 0 ? "#bfdbfe" : "#fed7aa",
                  boxShadow: isSelected ? "inset 0 1px 2px rgba(255,255,255,0.5)" : colors2000s.shadows.insetDark,
                  color: isSelected ? "#ffffff" : idx % 2 === 0 ? "#2563eb" : colors2000s.orange.dark,
                }}
              >
                {card.icon}
              </div>
              ...
              <div className="flex-1 min-w-0">
                <p className="text-lg font-black uppercase tracking-tight leading-none mb-2" style={{ color: isSelected ? "#ffffff" : colors2000s.text.primary }}>
                  {card.title}
                </p>
                <p className="text-[10px] font-black uppercase tracking-widest" style={{ color: isSelected ? "rgba(255,255,255,0.8)" : colors2000s.text.secondary }}>
                  {card.subtitle}
                </p>
              </div>
              ...
              <div
                ...
                style={{
                  background: isSelected ? "linear-gradient(180deg, #22c55e 0%, #16a34a 100%)" : "#ffffff",
                  borderColor: isSelected ? "#16a34a" : colors2000s.border.default,
                  boxShadow: isSelected ? "inset 0 1px 0 rgba(255,255,255,0.3)" : "inset 0 1px 2px rgba(0,0,0,0.1)",
                }}
              >
