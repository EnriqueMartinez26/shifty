...
import { AlertCircle, Calendar, CheckCircle2, ChevronLeft, Clock, ExternalLink, Loader2, Tag } from "lucide-react";
...
export const BookingStepConfirmation: React.FC<BookingStepConfirmationProps> = ({
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      setStatus("error");
      setErrorMessage(error.response?.data?.detail || "No pudimos procesar tu reserva. El horario podria estar ocupado.");
    }
  };
        ...
        >
          ...
          <div className="grid gap-4">
            ...
            {(confirmation?.service_price || confirmation?.final_price) && (
              <div className="rounded-2xl p-4 border" style={{ background: "#f8fafc", borderColor: "#cbd5e1" }}>
                <p className="text-[10px] font-black uppercase tracking-widest text-slate-600 mb-1">Resumen comercial</p>
                <div className="space-y-1 text-sm font-black text-slate-900">
                  ...
                  <p>Total final: {currencyFmt.format(Number(confirmation.final_price || confirmation.service_price || 0))}</p>
                </div>
              </div>
            )}
            ...
            {confirmation?.payment_required && (
              <div className="rounded-2xl p-4 border" style={{ background: "#fff7ed", borderColor: "#fed7aa" }}>
                ...
                <p className="text-sm font-black text-amber-900">
                  {confirmation.payment_amount ? currencyFmt.format(Number(confirmation.payment_amount)) : "Importe a confirmar"}
                </p>
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
      <div className="flex items-center gap-4 mb-2">
        ...
        <div>
          ...
          <p className="text-sm font-bold text-gray-500">Este es el ultimo paso antes de reservar.</p>
        </div>
      </div>

      <div className="rounded-3xl p-6 bg-white space-y-5" style={{ border: `1px solid ${colors2000s.border.light}`, boxShadow: colors2000s.shadows.insetDark }}>
        <div className="grid sm:grid-cols-2 gap-4">
          ...
          <div className="rounded-2xl p-4 border" style={{ borderColor: "#e5e7eb" }}>
            <p className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-1">Hora</p>
        ...
        <div className="rounded-2xl p-4 border space-y-3" style={{ borderColor: "#e5e7eb" }}>
          <div className="flex items-center gap-2">
            ...
            <p className="text-[10px] font-black uppercase tracking-widest text-gray-400">Codigo promocional</p>
          </div>
          ...
          {promotionPreview && (
            <div className="rounded-2xl p-4 border" style={{ background: "#ecfdf5", borderColor: "#bbf7d0" }}>
              <p className="text-[10px] font-black uppercase tracking-widest text-green-700">{promotionPreview.title}</p>
      ...
      {status === "error" && (
        <div className="rounded-2xl p-4 text-xs font-bold flex items-center gap-2" style={{ background: "#fff7ed", border: "1px solid #fed7aa", color: "#c2410c" }}>
          <AlertCircle className="w-4 h-4 flex-shrink-0" />
          El horario podria haberse ocupado mientras completabas el formulario. Volve un paso atras y elegi otro.
        </div>
      )}
    </div>
  );
