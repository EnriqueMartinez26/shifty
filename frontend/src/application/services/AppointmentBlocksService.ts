...
export class AppointmentBlocksService {
  ...
  async createRecurring(payload: RecurringAppointmentBlockPayload): Promise<RecurringBlocksResult> {
    const { data } = await apiClient.post<RecurringBlocksResult>("/appointment-blocks/batch", payload);
    return data;
  }

  async update(publicId: string, payload: Partial<AppointmentBlockPayload> & { is_active?: boolean }): Promise<AppointmentBlock> {
    const { data } = await apiClient.patch<AppointmentBlock>(`/appointment-blocks/${publicId}`, payload);
    return data;
  }
